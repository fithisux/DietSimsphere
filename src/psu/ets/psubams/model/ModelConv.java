 ...
 * @version %I%, %G%
 */
public class ModelConv implements Constants, SeriesNames {
	...
	public void _psubams(int iyr, int imo, int iday, double tz, double xlat, 
		 ...
		 double[] zh, double frvegstep, double fstep, double targettime, int no_row,
	...
	//	no_rows = (((int) timend - (int) strtim) / (int) out_time_intv) + 1;
		...
		frveg = frveg / 100;	/* Vegetation %age as a fraction */
                frvegstep = frvegstep / 100; //this as well...
		/**
		 * ******************************************
		 * end of calc
		 * ******************************************
	...
	zeta = zeta + dzeta;
        ...
        double finit = f; //initial moisture availability. This will iterate...
        do {//calculate values from strtim till targettime
             realtm = time + strtim; 
             ptime = realtm / 3600.0;
...
System.out.println("-- Ball");
	...
	// if (out_count < output_data[0].length){
        time += delta; //we increment time by 1:30 min.       
       }while (realtm < targettime); 
		...
		output_data[0][no_row] = frveg * 100;      //fractional vegetation cover
		output_data[1][no_row] = finit;   //current surface moisture availability
                output_data[2][no_row] = f;   //actual surface moisture availability
		output_data[3][no_row] = rnet;       //net radiation
		output_data[4][no_row] = evap;       //latent heat flux
		output_data[5][no_row] = heat;       //sensible heat flux
		output_data[6][no_row] = otemp - 273.23; //radiometric temperature		
