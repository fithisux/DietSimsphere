...
public class Model implements Constants, SeriesNames {
	...
	/* String[] output_names, */
	double[][] output_data, double[][] data_graphs, double[][][] sounding_data, 
                boolean batch, int dayOfTheYear/*update by Yannis Konstas*/) throws Exception {
...
//                System.out.println("day = "+ dayOfTheYear+ " xlat = "+ xlat+" edA = "+edA +" edB = "+edB + " edN = "+edN);
		...
		int stabcriteria = 0, init = 0, no_rows = 0, ionce = 0, monce = 0;
		 ...
		 * snding
		 * ********************************************?
		 * Read sounding and call spline to interpolate
		 */

		/* Data vertical spacing */
		vert_spacing = 250;	   /* 250 meter intervals */
		deltaz = vert_spacing;
		...
		tscren = ts[1] + C_TO_K;	/* Screen temperature */
		...
		/**
		 * Unused TCS
		 * satam=_dectim(clktam);
		 * satpm=_dectim(clktpm);
		 */
		...
		/**
		 * * This is the start of the diurnal loop (timend-strtim)
		 * Nominally 22 hours *
		 */
		...
		do {
                        ...
                        if (slope > 0) {
                                ...
                                sinsolelslope = Math.cos(dipan) * (Math.sin(slb) * Math.sin(effdec) 
                                                + Math.cos(slb) * Math.cos(effdec) * Math.cos(hrangl)) 
                                                + Math.sin(dipan) * (Math.cos(dipaz) * (Math.tan(slb) * (Math.sin(slb) * Math.sin(effdec) 
                                                + Math.cos(slb) * Math.cos(effdec) * Math.cos(hrangl)) - Math.sin(effdec) / Math.cos(slb)) 
                                                 + Math.sin(dipaz) * Math.cos(effdec) * Math.sin(hrangl));
                        ...
                        if (solel <= 0) {
                                ...
                                rlpath = (Math.pow((solsin + 0.15 * Math.pow(solel + 3.885, -1.253)), -1));
                        ...
                        if (frveg == 1) {
                                ...
                                vegnradiotemp = Math.pow(tzero4, 0.25);	   /* would be tzero */
                                rnetg = rlg + rsg;
                                rnetf = rlf + rsf;
                                vegnnetradn = rnetg + rnetf;	  /* Would be rnetv */
                                ...
                                vegnshortwave = (rsg + rsf);	  /* Would be swavev */
                        ...
                        if ((stabcriteria == 0) 
                                ...
                                ustar1 = _you_star(awind, za, zo, fm);	  /* Friction velocity */
                                ...
                                uten = _wind(ustar, zten, zo, ften);	  /* Surface winds at */
                                uscrn = _wind(ustar, reflev, zo, 
                                          0.0);		  /* screen level and 10m */
                                rzascr = _r_ohms(ustar, za, reflev, ft20);	  /* Resistances in */
                                rzazo = _r_ohms(ustar, za, zo, t_ft);	  /* surface layer */
                        ...
                        rtrans = _restrn(ustar, zo, kmm);	 /* resistance transition layer */
                ...
                System.out.println("-- Ball");
                                        ...
                                        rrtot = rrtot / 40.0;		  /* in mol units */
                
                                        /* Set some parameters photosyn model (now read in start */
                                        akco = 3.0e-4;
                                        akoo = 0.300;
                                        oxy = 0.209;
                                        vcmaxo = 2.0e-4;			  /* Collatz; 7.5e-5 data 1992 */
                                        alpi = 12;
                                        respo = 3.0e-6;
                                        tauo = 2600;
                                        ajpar = 380e-6;
                                ...
                                rst = rcut;
                                   ...
                                   * Math.pow(1 / (rst * 1.66 + raf * 1.32) + 1 
                                          / rair, -1);
                                        css = fco2 * raf * 1.66 + ccan;	   /* co2 at leaft surface */
                                        raf = raf * pes / xlai;
                                        rst = rst * pes / xlai;
                        ...
                        if (((heat >= 0) || (rnet > 0)) && (swave > 0)) {
                                ...
                                if ((frveg == 1) && (rnet > 0)) {
                                         ...
                                         * co2flx
                                         * ***********************
                                         * Created 11/2/90f; modified 06/17/91
                                         */
                         ...
                         */
                            ...
                            /*update by Yannis Konstas(end)*/    
                                ...
                                */
                                ...
                                if(wdir850 < 0) wdir850 = 360 + wdir850;
                                if(wdir500 < 0) wdir500 = 360 + wdir500;
                        ...
                        /* Increment time */
                        time += delta;
		} while (realtm < timend);		  /* End of the "do" statement */
	...
	*/
	...
	double p, t, td, esat, e, theta; /* Latent heat of evaporation */
	...
	if((ttt - tttd) > 0.01) /* Skip calculation if tttd~=ttt */
	{
		...
		{
			dp = dp/2.0;	/* Reduce pressure step */
			if((rns - rs) < 0) p = p + dp;
			if((rns - rs) >= 0) p = p - dp;
			t = theta / Math.pow(p0/p,xkap);
			rns = _wfun(_satvap(t,t), p);
